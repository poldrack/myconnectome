...
meancorsim=read.table(sprintf("%s/rsfmri/corrsim_mean.txt"))$V1
...
write.table(pvals,file=sprintf('%s/timeseries/tables/wincorr_timeseries_stats.txt',basedir))
...
names(pvals)=c('mean','var(resid)','Linear trend (p-val)','Poly trend (p-val)')
row.names(pvals)=names(datamat)[1:12]
write.table(pvals,file=sprintf('%s/timeseries/tables/bwcorr_timeseries_stats.txt',basedir))
kable(pvals)
if (save_latex) {
  k=kable(pvals,format='latex',digits=4)
